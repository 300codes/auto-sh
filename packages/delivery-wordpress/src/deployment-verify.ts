...
export async function verifyOwnedDeploymentPackage(value: unknown) {
  try {
    ...
    if (JSON.stringify((await fs.readdir(directory)).sort()) !== JSON.stringify(['manifest.json', 'site'])) throw toolError('deployment_verify_inventory_mismatch')
    ...
    const walk = async (folder: string, relative: string, depth: number): Promise<void> => {
      ...
      if (JSON.stringify((await fs.readdir(folder)).sort()) !== JSON.stringify(names)) throw toolError('deployment_verify_changed')
    }
    await walk(site, '', 0)
