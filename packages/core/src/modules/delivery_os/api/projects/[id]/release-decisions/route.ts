...
import { releaseDecisionSchema } from '@open-mercato/core/modules/delivery_os/data/validators'
...
import {
  ...
  releaseDecisionCreateResponseSchema,
} from '@open-mercato/core/modules/delivery_os/api/schemas'

export const metadata = {
  POST: { requireAuth: true, requireFeatures: ['delivery_os.release.approve'] },
}
...
export async function POST(request: Request, context: DeliveryRouteContext): Promise<Response> {
  try {
    ...
    const outcome = await executeDeliveryCommand<DecisionCommandResult>(ctx, scope, {
      ...
      body,
      pathInput: { projectId, kind: 'release' },
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      summary: 'Record the final acceptance (release decision) of one deployment',
      description:
        'Append-only and bound to one deployment evidence row of the active baseline. An approved verdict is accepted only when the deployment is verified, the latest publish consent for the same baseline and revision is approved and the delivery report is releasable on that revision; a reject is always allowed with a reason. Requires the project optimistic-lock header.',
      requestBody: { contentType: 'application/json', schema: releaseDecisionSchema },
      responses: [{ status: 201, description: 'Release decision recorded', schema: releaseDecisionCreateResponseSchema }],
      errors: [
        ...
        { status: 404, description: 'Project or deployment evidence not found in this scope', schema: deliveryErrorBodySchema },
        { status: 409, description: 'Stale project version, candidate or decision context', schema: z.union([optimisticLockConflictSchema, deliveryErrorBodySchema]) },
        {
          status: 422,
          description:
            'deployment_unverified, revision_mismatch (publish consent names another revision), deploy_decision_missing (none or the latest one rejected), report_not_green (details = release blockers, path <kind>:<id>, code = status), baseline_not_active, unsupported_evidence_kind (not a deployment row), deployment_incomplete, invalid_revision, reason_required, unknown_target_profile, hash_mismatch',
