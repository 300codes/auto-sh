...
import { deliveryReportWithFlowSchema, uuidSchema } from '@open-mercato/core/modules/delivery_os/lib/contracts'
...
export async function GET(request: Request, context: DeliveryRouteContext): Promise<Response> {
  try {
    ...
    return NextResponse.json(await queries.buildReport(scope, projectId, { ...query, includeFlow: true }))
  } catch (error) {
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      ...
      description:
        'Read-only: evaluates every acceptance criterion of the baseline (default: the active baseline) on one revision (default: the newest accepted result) and the project target profile, from stored evidence and decisions only. `revision` is `git:<commitSha>` or `snapshot:<sha256>:<externalWorkspaceId>`. Rows are capped by `limit` (default and maximum 1000) with `truncated`. Archived projects stay readable. A project pinned to a flow template additionally carries the optional `flow` section (F15: template ref, the four approval stages with currency, approved artifact and decision, and the stage gate); stage blockers appear only in `flow.gate`, never in the v1 `gates`. The flow section always reflects the current stage state, independent of `baselineId` / `revision`. Legacy projects answer the unchanged v1 body without `flow`.',
      query: reportQuerySchema,
      responses: [{ status: 200, description: 'DeliveryReport v1 (plus optional flow section)', schema: deliveryReportWithFlowSchema }],
      errors: [
