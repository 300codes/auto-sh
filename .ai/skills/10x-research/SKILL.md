...
2. **Analyze and decompose the research question:**
   ...
   - List the research areas explicitly in your reply before spawning agents, and report each one as its findings land
...
3. **Clarify research scope using AskUserQuestion**:
   ...
   **Example** — for an ambiguous query like "how does authentication work":
   ...
   For a clear, scoped query like "find all files using the apiCall helper":
...
4. **Spawn parallel sub-agent tasks for comprehensive research:**
   ...
   Use the `Agent` tool with parallel sub-agents:
...
- Use parallel `Agent` sub-agents for efficiency — main agent synthesizes, sub-agents do deep reading
...
- **Progress tracking**: Name the research areas up front and report each one back as it completes, so the user can follow progress
