...
| `status_reason` | text null | e.g. `reconciliation_required`, `dependency_blocked` |
...
| `delivery_os.baseline.approved` | `{ projectId, baselineId, version, contentHash, activeBaselineId }` | emitted when `activeBaselineId` changes |
| `delivery_os.task.updated` | `{ projectId, taskId, status, statusReason, updatedAt }` | `clientBroadcast: true` for live status |
| `delivery_os.evidence.recorded` | `{ projectId, taskId, attemptId, evidenceId, kind, duplicate, completionDelivery }` | re-emitted on duplicate result import so enterprise retries a pending delivery; `clientBroadcast: true` |
