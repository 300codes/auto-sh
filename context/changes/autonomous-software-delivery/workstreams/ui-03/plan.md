...
- [ ] 1.1 Testy modułu przechodzą, w tym nowe testy `proposalImport` dla pięciu ścieżek wejścia
- [ ] 1.2 Test dialogu rozróżnia `200 duplicate` od `409 idempotency_conflict`
- [ ] 1.3 `i18n:check-sync`, `i18n:check-usage` i `i18n:check-hardcoded` są czyste
- [ ] 1.4 Fixture `design-manifest.v1.json` parsuje się `designManifestV1Schema` w teście
- [ ] 1.5 Granica własności `{api,commands,data,lib}` jest pusta
...
- [ ] 2.1 Testy `screenUpload`: hash referencyjny, brak `crypto.subtle`, zły typ i za duży plik
- [ ] 2.2 Test mapera `DesignManifest → ScreenRef[]`: brak pliku dla wpisu jest błędem
- [ ] 2.3 Test zapisu draftu: nieparsowalny `draftSpec` blokuje `PUT`
- [ ] 2.4 Test `FreezeBaselineAction`: cztery rozłączne komunikaty dla czterech wyników
- [ ] 2.5 Test `DesignSection`: nieudane wczytanie obrazu daje nazwany stan, metadane zostają
- [ ] 2.6 Testy modułu, `i18n:*`, `typecheck`, `lint` przechodzą; granica własności pusta
...
- [ ] 3.1 Test wyboru wersji: domyślna, najnowsza przy braku aktywnej, martwe `?baselineId=` czyszczone
- [ ] 3.2 Test `DecisionActions`: odrzucenie bez powodu nie wysyła żądania; hash i wersja z oglądanej wersji
- [ ] 3.3 Test rozłączności `subject_hash_mismatch` i `optimistic_lock_conflict`
- [ ] 3.4 Test F1: `hasActiveBaseline === null` przy `state: 'ready'` nie twierdzi „brak baseline'u"
- [ ] 3.5 Testy modułu, `i18n:*`, `typecheck`, `lint` przechodzą; granica własności pusta
...
- [ ] 4.1 Testy `proposalImport` dla planu: cykl, nieznane AC, `allowedPaths`, mapowanie na nieistniejący test
- [ ] 4.2 Test dialogu planu: `details[]` renderują się jako lista wszystkich ścieżek
- [ ] 4.3 Test komunikatu po imporcie zawiera stwierdzenie o nieaktywnym scalonym baseline
- [ ] 4.4 `TC-DELIVERY-UI-003` istnieje, przechodzi `typecheck` i `lint`, teardown adresuje projekty po id
- [ ] 4.5 Pełna bramka OM przechodzi na wybranym runnerze
- [ ] 4.6 `yarn check:client-boundaries` nie zgłasza nowych naruszeń
- [ ] 4.7 Granica własności `{api,commands,data,lib}` jest pusta
- [ ] 4.8 Brak zmian w plikach `*.generated.*`; żaden nowy komponent kliencki nie przekracza 300 LOC
