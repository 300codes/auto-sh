...
Plan pracy integracyjnej: przegląd trzech branchy → scalenie w izolowanym worktree → poprawki i kontrole kontraktów → gate na wspólnej rewizji → push do main po pozytywnej walidacji. Osobny [raport integracji](../overnight-integration-2026-09-19.md) zapisuje faktyczny wynik publikacji i testów.

## Co jest już w kodzie

| Strumień | Przeglądany head | Rezultat | Granica ukończenia |
|---|---|---|---|
| OSS / Mateusz | `dev-mateusz` — `c24ac8ba8` | Kontrakty v1, encje, scoped API, baseline/decisions/proposals, reserve/claim/link/result/cancel/reconcile/evidence | Wynik kończy na awaiting_review; review i późniejsze etapy nie mają jeszcze obsługi |
| UI / Adam | `feature/design-ui` — `9a08a7952` | Readiness, skrypty Figmy i dołączony podczas integracji ekran szczegółów z hostem execution | OAuth/write/update nadal blocked; lista projektów i baseline UI pozostają do zrobienia |
...
|---|---|---|---|
| **Mateusz — OSS** | Dokończyć OSS-04: review evidence i przejścia verified/changes_requested, optimistic lock, scope, idempotencja | R22 report, R20 deploy-decisions i R21 release-decisions | result → review → verified odblokowuje zależne zadanie; cancel → potwierdzone stop → nowa próba; testy odmów i duplikatów |
